export default function ThemeToggle({ isDark, onToggle, className = '' }) {
    return (
        <button
            ...
            className={`inline-flex items-center justify-center w-10 h-10 rounded-lg border border-border bg-panel text-sub transition-colors hover:text-brand hover:border-brand ${className}`}
        >
            {isDark ? (
